...
class Visitor {
 ...
 public:
  ...
  void rand_write_page(uint32_t page_no) {
    ...
    //std::cout<<page_no<<"这一页应该的checksum是："<<checksum<<std::endl;
  ...
  void check_read_page(uint32_t page_no) {
    ...
    /*现在才去检查buffer*/
    uint32_t checksum = page_checksum(buf);
    //std::cout<<page_no<<"这一页实际的checksum是："<<checksum<<std::endl;
...
int main(int argc, char* argv[]) {
  assert(argc <= 2);
  srand(time(NULL));
  //刚好写了一个G的数据，1GB/16KB，就是65536个页，那说明就是追加写的
  ...
  for (uint32_t page_no = 0; page_no < page_cnt; page_no++) {
    visitor.rand_write_page(page_no);/*这里面是先塞满缓冲区，这样弄得，后续的run_trace还需要研究一下是不是先导入数据。*/
